"""
Script corrigido para testar a câmera 1 (que está funcionando) e detecção de gaze.
"""
...
def test_camera():
    """Testa se a câmera 1 está funcionando."""
    print("📷 Testando câmera 1...")
    
    # Usar câmera 1 que detectamos funcionando
    cap = cv2.VideoCapture(1)
    
    if not cap.isOpened():
        print("❌ Erro: Não foi possível abrir a câmera 1")
        return False
    ...
    if ret:
        print(f"✅ Câmera 1 funcionando! Frame capturado: {frame.shape}")
        cap.release()
        return True
    else:
        print("❌ Erro: Não foi possível capturar frame da câmera 1")
        cap.release()
        return False
...
def capture_and_detect():
    """Captura um frame da câmera 1 e testa a detecção de gaze."""
    print("🎯 Capturando frame da câmera 1 e testando detecção...")
    
    # Capturar frame da câmera 1
    cap = cv2.VideoCapture(1)
    cap.set(cv2.CAP_PROP_FRAME_WIDTH, 1280)  # Usar resolução nativa
    ...
    cv2.imwrite("test_frame_camera1.jpg", frame)
    print("💾 Frame salvo como 'test_frame_camera1.jpg'")
    ...
    try:
        ...
        data = {'session_id': 'test_session_camera1'}
...
def main():
    """Função principal."""
    print("🚀 Teste de Câmera 1 e Detecção de Gaze")
    print("=" * 50)
    
    # Testar câmera 1
    if not test_camera():
        print("❌ Teste da câmera 1 falhou.")
        return
